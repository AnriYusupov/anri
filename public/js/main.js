...
/* Запустить ролик заново с начала. Годится и для обычного файла, и для потока:
   если источник ещё не готов, отметка __vwant заставит запустить его сразу,
...
function setVideoSrc(el, url, autoplay){
  if(!el || !url) return;

  /* ГРАБЛИ. Обычный файл ставится мгновенно: написал el.src — можно играть.
     А поток подключается через плеер, и это занимает время: сначала надо
     подгрузить сам плеер, потом список дорожек, потом первый кусок.
     Вызывающий код этого не знал и звал play() сразу следующей строкой —
     когда играть ещё нечего. Отсюда «то показывает, то нет».
     Поэтому теперь запуск живёт здесь: кто просит autoplay, тот получит
     воспроизведение тогда, когда оно станет возможным, а не раньше. */
  var token = (el.__vtok = (el.__vtok || 0) + 1);
  if(autoplay) el.__vwant = true;

  /* Пока мы возились с одним роликом, мышь могла уехать на другой кейс
     и запустить второй заход. Тогда этот, устаревший, должен молча уйти:
     иначе к одному окошку прицепятся два плеера сразу. */
  function stale(){ return el.__vtok !== token; }

  function start(){
    if(stale() || !el.__vwant) return;
    try{ el.currentTime = 0; }catch(e){}
    var p = el.play();
    if(p && p.catch) p.catch(function(){});
  }
  ...
  if(!isStream(url)){ plain(); return; }

  loadHls().then(function(Hls){
    if(stale()) return;

    /* Раньше здесь первым делом спрашивали браузер: «умеешь играть поток сам?»
       Задумка была про айфоны — у них встроенный проигрыватель лучше нашего.
       Но Chrome на этот вопрос тоже отвечает «умею», забирает поток себе
       и играет с самой мутной дорожки: 426×240 в окне на тысячу пикселей.
       Наши настройки при этом не применялись вовсе — плеер не создавался.
       Теперь наоборот: свой плеер везде, где он работает, потому что только
       его мы можем настроить. Встроенный — запасной, для iPhone. */
    if(!Hls || !Hls.isSupported()){ plain(); return; }

    var h = new Hls({
      maxBufferLength: 12,          /* не набираем лишнего впрок */
      abrEwmaDefaultEstimate: 10000000,   /* канал считаем быстрым, а не полудохлым */
    ...
    /* Качество выбираем сами. Смотрим высоту окошка на экране, берём первую
       дорожку, которая в него влезает, и поднимаемся на ступень выше — чтобы
       было с запасом и не мылило. В маленьком превью 4K не нужен, он там
       всё равно не виден, а трафик съест.

       И главное: ставим уровень ЖЁСТКО, а не подсказкой. Подсказку плеер
       вправе проигнорировать и всё равно начать снизу — отсюда и оставались
       те несколько секунд каши в начале. Через восемь секунд отпускаем
       обратно на автомат: к этому времени зритель уже увидел работу
       как надо, а дальше пусть подстраивается под канал. Если раньше того
       начнутся заикания — отпускаем немедленно, лучше мягче, чем рывками. */
      ...
      if(L.length){
        ...
        if(need > 0){
          for(var i = 0; i < L.length; i++){
            if((L[i].height || 0) >= need){ pick = Math.min(i + 1, L.length - 1); break; }
          }
        }
        h.startLevel = pick;
        h.currentLevel = pick;
        setTimeout(function(){
          if(el.__hls === h){ try{ h.currentLevel = -1; }catch(e){} }
        }, 8000);
      }
      start();
    });
    ...
    h.on(Hls.Events.ERROR, function(evt, data){
      if(data && (data.details === "bufferStalledError" || data.details === "bufferNudgeOnStall")){
        try{ h.currentLevel = -1; }catch(e){}   /* не тянет — отдаём автомату сразу */
      }
    });
    ...
    h.attachMedia(el);
    el.__hls = h;
  }).catch(function(){ plain(); });   /* плеер не загрузился — пусть браузер пробует сам */
}
...
function hideApp(){
  ...
  window.showQL = function(proj){
    ...
    if(src){
      ...
      if(qlPlayer.getAttribute("data-src") !== src){
        ...
        /* Запуск отдаём setVideoSrc. У потока источник появляется не сразу,
           и звать play() следующей строкой — значит просить играть пустоту:
           браузер молча откажет, и окошко останется чёрным до нового наведения.
           Отдельный load() тоже убран — он сбрасывал уже подключённый плеер. */
        setVideoSrc(qlPlayer, src, true);
      } else {
        playVideo(qlPlayer);
      }
      qlPlaying = false;
  ...
  window.hideQL = function(){
    qlTimer = setTimeout(function(){
      ...
      qlPlaying = false;
      if(qlRaf) cancelAnimationFrame(qlRaf);
      /* Снимаем просьбу играть: иначе поток, который ещё подключается,
         бодро запустится уже после того, как мышь ушла. */
      qlPlayer.__vwant = false;
      try{ qlPlayer.pause(); }catch(e){}
    }, 90);
  };
